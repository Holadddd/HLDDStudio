...
class DrumMachinePattern {
    ...
    init(DrumBeatPattern: DrumBeatPattern,
         drumType: DrumType,
         fileIndex: Int) {
...
class DrumMachineManger {
    ...
    func creatPattern(withType: DrumType,
                      drumBeatPattern: DrumBeatPattern,
                      fileIndex: Int){
        
        pattern.append(DrumMachinePattern(DrumBeatPattern: drumBeatPattern,
                                          drumType: withType,
                                          fileIndex: fileIndex))
        
        let patternIndex = pattern.count - 1
        
        drumMixer.connect(input: pattern[patternIndex].node,
                          bus: patternIndex)
        
        creatCoreData(withType: withType,
                      drumBeatPattern: drumBeatPattern,
            fileIndex: fileIndex)
    }
    
    func copyPatternFromCore(withType: DrumType,
                             drumBeatPattern: DrumBeatPattern,
                             fileIndex: Int){
    ...
    func changeDrumSample(atRow: Int,
                          withType: DrumType,
                          fileIndex: Int) {
    ...
    func playDrumMachine() {
        ...
        timer = Timer.scheduledTimer(timeInterval: (60 * 4 / 8)/bpm,
                                     target: self,
                                     selector: #selector(drumMachineSetAndPlay),
                                     userInfo: nil,
                                     repeats: true)
    }
        ...
        for (index, drumPatterm) in pattern.enumerated() {
            ...
            DispatchQueue.main.async {
                ...
                if drumPatterm.drumBeatPattern.beatPattern[beats]{
                    ...
                    NotificationCenter.default.post(name: .drumMachinePatternAnimation,
                                                    object: DrumMachinePatternAnimationInfo(indexPath: indexPath,
                                                                                            startTime: start))
    ...
    func mixerPlayDrumMachine() {
        ...
        timer = Timer.scheduledTimer(timeInterval: (60 * 4 / 8)/mixerBpm,
                                     target: self,
                                     selector: #selector(drumMachineSetAndPlayAtMixer),
                                     userInfo: nil,
                                     repeats: true)
    }
...
extension DrumMachineManger {
    ...
    func creatCoreData(withType: DrumType,
                       drumBeatPattern: DrumBeatPattern,
                       fileIndex: Int) {
        ...
        case .classic:
            ...
            guard let fileIndex = classicFileNameArr.firstIndex(of: pattern[patternIndex].fileName)
                else { fatalError() }
        ...
        case .hihats:
            ...
            guard let fileIndex = hihatsFileNameArr.firstIndex(of: pattern[patternIndex].fileName)
                else { fatalError() }
        ...
        case .kicks:
            ...
            guard let fileIndex = kicksFileNameArr.firstIndex(of: pattern[patternIndex].fileName)
                else { fatalError() }
        ...
        case .percussion:
            ...
            guard let fileIndex = percussionFileNameArr.firstIndex(of: pattern[patternIndex].fileName)
                else { fatalError() }
        ...
        case .snares:
            ...
            guard let fileIndex = snaresFileNameArr.firstIndex(of: pattern[patternIndex].fileName)
                else { fatalError() }
