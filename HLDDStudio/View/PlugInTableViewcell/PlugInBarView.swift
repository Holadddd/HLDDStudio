...
protocol PlugInControlDelegate: AnyObject {
    ...
    func plugInBypassSwitch(_ isBypass: Bool,
                            cell: UITableViewCell)
    
    func plugInReverbFactorySelect(_ factoryRawValue: Int,
                                   cell: PlugInReverbTableViewCell)
    
    func dryWetMixValueChange(_ value: Float,
                              cell: UITableViewCell)
    
    func guitarProcessorValueChange(_ value: Float,
                                    type: GuitarProcessorValueType,
                                    cell: UITableViewCell)
    
    func delayValueChange(_ value: Float,
                          type: DelayValueType,
                          cell: UITableViewCell)
    
    func chorusValueChange(_ value: Float,
                           type: ChorusValueType,
                           cell: UITableViewCell)
}
...
class PlugInBarView: UIView {
    ...
    private func initPlugInBarView() {
        ...
        bypassButton.addTarget(self,
                               action: #selector(PlugInBarView.bypassButtonAction),
                               for: .touchUpInside)
    }
...
extension PlugInBarView: UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView,
        ...
        guard let preset = datasource?.presetParameter()
            else { return }
...
extension PlugInBarView: UITextFieldDelegate {
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        guard let parameter = textField.text
            else { return }
